/**
 * POST /api/feedback/feature — Build 002
 *
 * Validates preview token → auto-extracts title from description →
 * uploads screenshot → inserts into `features` with source='widget'.
 */
...
export default async (req: Request) => {
  ...
  // Auto-extract title: first sentence, max 60 chars, trimmed
  const desc      = body.description as string;
  const autoTitle = desc.split(/[.!?\n]/)[0].trim().slice(0, 60) || 'New feature';
  ...
  const { data, error } = await supabaseAdmin
    ...
    .insert({
      ...
      name:           autoTitle,
      description:    desc,
      status:         'design',   // always starts in design stage
      complexity:     'medium',   // default; owner can adjust
      source:         'widget',
...
export const config = { path: '/api/feedback/feature' };
